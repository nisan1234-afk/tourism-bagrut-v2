...
**Correction (27.08.2026, after user pushback — this section was wrong the first time):** the original version of this section concluded "no API for NotebookLM" → "so it requires a manual step by Nisim." That inference doesn't hold. **"No API" and "not automatable" are not the same thing when the agent has real Chrome browser control** — this project's own "B" session has been driving arbitrary web UIs all session (the Apps Script web editor's CodeMirror search/replace, GitHub's web commit UI, GitHub Pages verification) with no API for any of those either, just clicks/typing/navigation. There is no evidence-based reason B couldn't do the same in NotebookLM's UI — upload sources, click the Studio panel's "Generate" buttons, download/copy the output — if Nisim grants it access to the Google account NotebookLM runs under. **This is untested, not impossible.** The user separately reports Codex also has browser control available to it (not something I independently verified in this research pass, but plausible and worth taking at face value pending an actual test).

- **Claude (via the "B" session)** — proven in this project: real Chrome control already used for Apps Script deploys and live QA. Extending that to NotebookLM (upload + click Studio-panel buttons) is a small, testable step, not a new capability.
- **Codex** — has documented *codebase/terminal* agent autonomy (clone repo, edit files, run tests, open PRs — see §3). Separately, per the user, it also has browser control available in this environment; if so, it could in principle operate NotebookLM the same way B can, while still staying under the existing rule that it never touches the site's repo (browser use and repo-write are separate permissions, not the same thing).
- **NotebookLM itself** — still no first-party API found. But that only rules out *programmatic* automation (a script calling an endpoint) — it does not rule out an agent with real browser control clicking through the same UI a human would use.
- **Gemini app** — same "no API found ≠ not browser-automatable" correction applies, though nothing in this research or the conversation confirms either B or Codex has been pointed at the Gemini app specifically.

**Practical upshot:** the earlier claim that NotebookLM-based ideas (Audio Overview, Mind Map, Infographic, Flashcards — §"רעיונות לשיפור" below) "will remain a manual step for the teacher" was too strong. If B (or Codex) is given account access, it can very plausibly drive the upload-and-click workflow itself, with the human role reduced to a one-time access grant rather than repeated manual operation. **This should be tested directly**, not assumed either way — a good first test would be a single small NotebookLM task via B (e.g. generate one Mind Map from one source file) to confirm the UI is actually navigable by the agent before relying on it for real region-unit work.
