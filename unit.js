const steps=['overview','north','carmel','telaviv','south','images','presentation','practice','open-practice'];
...
{title:'נוף ותיירות בדרום',question:'א. תארו את הנוף האופייני למישור החוף המרכזי והדרומי. ב. ציינו שני אתרי תיירות מיוחדים באזור והסבירו מה מייחד אותם.',model:'הנוף מישורי, קו החוף ישר וארוך, החוף רחב וחולי והרצועה מגיעה עד כ־40 ק״מ. גן לאומי תל אשקלון מיוחד בשער הכנעני ובשילוב ארכאולוגיה וחוף; בית גוברין מיוחד במערות הפעמון, הקולומבריום והמקוואות.'},
{title:'יפו והעיר הלבנה',question:'ציינו שני מוקדי תיירות ביפו והסבירו את ייחודם. לאחר מכן הסבירו מדוע העיר הלבנה הוכרה כאתר מורשת עולמית.',model:'נמל יפו הוא נמל דייגים פעיל המשלב היסטוריה ואווירה ים־תיכונית; שוק הפשפשים מציע יד שנייה, אמנות וחוויה עממית. העיר הלבנה כוללת יותר מ־4,000 מבנים בסגנון הבינלאומי ותכנון עיר גנים.'},
{title:'תכנון סיור היסטורי',question:'תכננו סיור בן יומיים היוצא מנהרייה וכולל שישה אתרים — שלושה בכל יום. ציינו ליד כל אתר את סוגו ואת משך הביקור.',model:'יום א׳: ראש הנקרה — טבע, 90 דקות; אולמות האבירים — ארכאולוגיה, שעתיים; מסגד אל־ג׳זאר — דת, 45 דקות. יום ב׳: מונפורט — היסטוריה וטבע, שעתיים; יחיעם — היסטוריה, 90 דקות; אכזיב — טבע ונופש, שעה.'}
];
...
openSection.innerHTML=`<div class="block-heading"><span class="number">09</span><div><span class="eyebrow">תרגול פתוח</span><h2>כותבים כמו בבגרות</h2></div></div><p class="practice-intro">הטיוטות נשמרות במכשיר הזה. כדי להשלים את השלב יש לענות תשובה מפורטת לכל שלוש השאלות.</p><div class="open-question-list">${openPracticeData.map((q,i)=>`<article class="open-question"><span>שאלה ${i+1}</span><h3>${q.title}</h3><p>${q.question}</p><textarea data-open-answer="${i}" rows="7" placeholder="כתבו כאן תשובה מלאה: ציון האתר, עובדה והסבר..."></textarea><div class="answer-meta"><small data-word-count="${i}">0 מילים</small><small>מומלץ: לפחות 40 מילים</small></div><details><summary>מחוון ותשובת דוגמה</summary><div><b>בדיקה עצמית:</b><ul><li>עניתי על כל חלקי השאלה</li><li>ציינתי שמות מדויקים של אתרים</li><li>לכל אתר הוספתי עובדה והסבר</li></ul><p>${q.model}</p></div></details></article>`).join('')}</div><div class="open-actions"><button class="button button-primary" id="saveOpenPractice">שמירה ובדיקת השלמה</button><p id="openPracticeFeedback" class="quiz-feedback"></p></div>`;
practiceSection.parentNode.insertBefore(openSection,practiceSection.nextSibling);const practiceNav=document.querySelector('[data-step="practice"]');const openNav=document.createElement('a');openNav.href='#open-practice';openNav.dataset.step='open-practice';openNav.innerHTML='<i>9</i><span>שאלות פתוחות<small>כתיבה לבגרות</small></span><b>✓</b>';practiceNav.parentNode.appendChild(openNav);
const openDrafts=JSON.parse(localStorage.getItem('coastal-open-drafts')||'{}');document.querySelectorAll('[data-open-answer]').forEach(t=>{t.value=openDrafts[t.dataset.openAnswer]||'';const update=()=>{const words=t.value.trim()?t.value.trim().split(/\s+/).length:0;document.querySelector(`[data-word-count="${t.dataset.openAnswer}"]`).textContent=`${words} מילים`};t.addEventListener('input',update);update()});
document.getElementById('saveOpenPractice').addEventListener('click',()=>{const values=[...document.querySelectorAll('[data-open-answer]')].map(t=>t.value.trim());const saved={};values.forEach((v,i)=>saved[i]=v);localStorage.setItem('coastal-open-drafts',JSON.stringify(saved));const ready=values.every(v=>v.split(/\s+/).filter(Boolean).length>=25);document.getElementById('openPracticeFeedback').textContent=ready?'הטיוטות נשמרו והשלב הושלם. אפשר להמשיך לשפר אותן בכל עת.':'הטיוטות נשמרו. כדי להשלים את השלב, כתבו לפחות 25 מילים בכל תשובה.';if(ready)completeStep('open-practice')});
