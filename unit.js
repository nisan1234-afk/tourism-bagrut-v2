...
openSection.innerHTML=`<div class="block-heading"><span class="number">09</span><div><span class="eyebrow">שאלות בגרות מחומרי המקור</span><h2>מאגר שאלות מישור החוף</h2></div></div><p class="practice-intro">תשע שאלות מתוך הקובץ המקורי שבתיקיית חומרי הגלם. לא צוינה בקובץ שנת בחינה, ולכן הן אינן מתויגות לפי שנה. הטיוטות נשמרות במכשיר הזה.</p><div class="bank-progress"><b id="bankCompleted">0 מתוך 9 הושלמו</b><div class="meter"><i id="bankMeter"></i></div></div><div class="open-question-list">${openPracticeData.map((q,i)=>`<article class="open-question" data-open-card="${i}"><div class="question-label"><span>שאלה ${i+1}</span><em>${q.topic}</em></div><h3>${q.title}</h3><p>${q.question}</p><textarea data-open-answer="${i}" rows="8" placeholder="כתבו כאן תשובה מלאה לכל הסעיפים..."></textarea><div class="answer-meta"><small data-word-count="${i}">0 מילים</small><small data-answer-state="${i}">טרם הושלם</small></div><details><summary>מחוון ותשובת דוגמה</summary><div><b>בדיקה עצמית:</b><ul><li>עניתי על כל סעיפי השאלה</li><li>ציינתי שמות מדויקים של אתרים</li><li>לכל אתר הוספתי עובדה והסבר</li></ul><p>${q.model}</p></div></details></article>`).join('')}</div><div class="open-actions"><button class="button button-primary" id="saveOpenPractice">שמירת כל הטיוטות</button><p id="openPracticeFeedback" class="quiz-feedback"></p></div>`;
