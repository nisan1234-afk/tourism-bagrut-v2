...
function completeStep(step) {
  ...
  renderProgress();
  // דיווח התקדמות דפים לשרת, כמו ביחידות על המנוע המשותף (E4)
  bagrutAPI('saveBagrutUnitProgress', { unit_id: BAGRUT_UNIT_ID, pages_done: completed.size, page_total: steps.length }).catch(() => {});
}
document
...
<article class="learning-game wide-game" data-game-card="memory"><span>משחק 4</span><h3>זיכרון: אתר ועובדה</h3><p>הפכו שני קלפים ומצאו את הזוג המתאים.</p><div class="memory-board" id="memoryBoard"></div><small id="memoryFeedback"></small><button class="button button-outline" id="resetMemory">ערבוב מחדש</button></article>
<article class="learning-game" data-game-card="concepts"><span>משחק 5</span><h3>מושג והסבר</h3><p>התאימו לכל מושג את ההסבר המדויק.</p><div id="conceptGame"></div><button class="button button-outline" id="checkConcepts">בדיקה</button><small id="conceptFeedback"></small></article>
<article class="learning-game" data-game-card="puzzle"><span>משחק 6</span><h3>פאזל תמונה</h3><p>בחרו שתי חתיכות כדי להחליף ביניהן ולהרכיב את התמונה.</p><div class="image-puzzle" id="imagePuzzle"></div><small id="puzzleFeedback"></small><button class="button button-outline" id="shufflePuzzle">ערבוב מחדש</button></article>
<article class="learning-game" data-game-card="map"><span>משחק 7</span><h3>על מפת ישראל</h3><p id="mapPrompt">מצאו את האתר המבוקש על המפה.</p><div class="coast-map" id="coastMap"></div><a class="map-credit" href="https://commons.wikimedia.org/wiki/File:Israel_location_map_current_borders.svg" target="_blank" rel="noopener">מפה: Wikimedia Commons</a><small id="mapFeedback"></small><button class="button button-outline" id="resetMap">משחק חדש</button></article>
...
const questions = [
  // 20 שאלות הבוחן המאושרות של מישור החוף, הועברו מ-tourism11 (04.09.2026)
  ["מהו אורכו הכולל של מישור החוף (מראש הנקרה ועד רצועת עזה)?", ["90 ק\"מ", "190 ק\"מ", "290 ק\"מ", "390 ק\"מ"], 1, "אורך מישור החוף הוא כ-190 ק\"מ, ברוחב שבין 4 ק\"מ בצפון ל-40 ק\"מ בדרום."],
  ["איזה סוג חוף אופייני לצפון מישור החוף?", ["חוף צבירה", "חוף סחיפה", "חוף אלמוגים", "חוף ביצתי"], 1, "בצפון, גלים מכרסמים את רכסי הכורכר ויוצרים חוף סחיפה עם מפרצונים ולוחות גידוד. בדרום, לעומת זאת, החוף הוא חוף צבירה — רחב, ישר וחולי."],
  ["באיזו שנה נבנה מסגד אל-ג'זאר בעכו?", ["1517", "1781", "1948", "2003"], 1, "מסגד אל-ג'זאר נבנה ב-1781 על ידי הפאשא אחמד אל-ג'זאר, ובנייתו כללה עמודי גרניט שהובאו מאיטליה."],
  ["מהם שני האתרים הקדושים לבהאים בישראל?", ["הכותל המערבי וקבר רחל", "הגנים הבהאיים בחיפה וארמון בהג'י בעכו", "מסגד אל-אקצא וכנסיית הקבר", "מצדה וקומראן"], 1, "הבהאות היא דת שנוסדה בפרס לפני כ-150 שנה. שני אתריה הקדושים בישראל הם הגנים הבהאיים בחיפה (אתר UNESCO) וארמון בהג'י בעכו, שם קבור מייסד הדת."],
  ["באיזו שנה הוכרזה \"העיר הלבנה\" בתל אביב כאתר מורשת עולמית של UNESCO?", ["1948", "1993", "2003", "2013"], 2, "ב-2003 הכריז ארגון UNESCO על \"העיר הלבנה\" בתל אביב — הריכוז הגדול בעולם של מבנים בסגנון הבינלאומי (באוהאוס) — כאתר מורשת עולמי."],
  ["מי בנה את העיר קיסריה?", ["שלמה המלך", "המלך הורדוס", "הצלבנים", "העות'מאנים"], 1, "קיסריה נבנתה על ידי המלך הורדוס לכבוד הקיסר הרומי, והיא כוללת תיאטרון רומי, אקוודוקט והיפודרום השמורים היטב."],
  ["מה הכינוי של גן לאומי בית גוברין-מרשה, שהוכרז אתר מורשת עולמי?", ["\"עמק האלה\"", "\"ארץ אלף המערות\"", "\"שפלת המלכים\"", "\"גבעת הזהב\""], 1, "הגן נקרא \"ארץ אלף המערות\" בזכות אלפי המערות שנחצבו בקרקע הגירית — ובהן מערות פעמון, קולומבריום ובריכות טבילה."],
  ["מהם הגבולות הגיאוגרפיים של מישור החוף הצפוני?", ["בין הכרמל לרצועת עזה", "בין ראש הנקרה לכרמל", "בין תל אביב לאשקלון", "בין הירדן לים התיכון"], 1, "מישור החוף הצפוני משתרע בין ראש הנקרה בצפון (גבול ישראל-לבנון) לבין הכרמל בדרום, בין הרי הגליל במזרח לים התיכון במערב."],
  ["מה רוחבו של מישור החוף בצפון לעומת הדרום?", ["4 ק\"מ בצפון, 40 ק\"מ בדרום", "40 ק\"מ בצפון, 4 ק\"מ בדרום", "רוחב אחיד של 20 ק\"מ לכל אורכו", "10 ק\"מ בכל אורכו"], 0, "מישור החוף מתרחב מצפון לדרום: כ-4 ק\"מ בצפון (בסביבת ראש הנקרה) עד כ-40 ק\"מ בדרום."],
  ["כיצד נוצרו הנקרות בראש הנקרה?", ["נחצבו בידי אדם לצורכי הגנה", "מערות גיאולוגיות שנוצרו מגלי הים המכים בסלע הגיר", "שרידי מנהרות צבאיות", "תוצאה של רעידות אדמה"], 1, "הנקרות הן תופעת טבע — מערות שנוצרו לאורך אלפי שנים מפגיעת גלי הים בסלע הגיר הרך."],
  ["לכמה דתות קדושה מערת אליהו הנביא בחיפה?", ["לדת אחת בלבד", "לשתי דתות", "לשלוש דתות", "לארבע דתות"], 3, "מערת אליהו הנביא קדושה לארבע דתות: יהודים, נוצרים, מוסלמים ודרוזים."],
  ["מה מייחד את בית לוחמי הגטאות בגליל המערבי?", ["מוזיאון ספורט ישראלי", "מוזיאון שואה שנוסד על ידי ניצולים שהיו לוחמים בגטאות", "מוזיאון למלחמת העצמאות", "מוזיאון לתרבות ימית"], 1, "בית לוחמי הגטאות הוא מוזיאון שואה ייחודי, שנוסד על ידי ניצולי שואה שהיו בעצמם לוחמים בגטאות."],
  ["מה ניתן לראות בגן לאומי מבצר מונפורט?", ["שרידי מבצר צלבני של המסדר הגרמני בלב שמורת נחל כזיב", "מקדש בהאי", "נמל דייגים עתיק", "מכרות נחושת קדומים"], 0, "מונפורט הוא מבצר צלבני של המסדר הגרמני, השוכן בלב שמורת נחל כזיב ומציע נוף מרהיב."],
  ["לשם מה שימש האקוודוקט בקיסריה?", ["הגנה על העיר מפני פולשים", "הובלת מים לעיר ממעיינות הכרמל, כ-20 ק\"מ", "תעלת שיט לספינות מסחר", "אחסון תבואה"], 1, "האקוודוקט הוא אמת מים רומית שהובילה מי שתייה לקיסריה ממרחק של כ-20 ק\"מ מהכרמל, והוא שמור יפה על קו הים."],
  ["מה ניתן לעשות בזיכרון יעקב?", ["לצפות בתיאטרון רומי", "לבקר בבית אהרנסון, לסייר ביקבים (\"דרך היין\") ולבקר ברמת הנדיב", "לשוט בסירות באגם מלאכותי", "לצלול בין שרידי אונייה"], 1, "זיכרון יעקב מציעה את בית אהרנסון (מטה ניל\"י), סיורי יין ביקבי כרמל, הברון ואליעז, ואת גני הזיכרון ברמת הנדיב."],
  ["מה מציעים הכפרים הדרוזיים דלית אל-כרמל ועוספיה לתיירים?", ["ספורט ימי וצלילה", "שוק ססגוני, מטבח דרוזי אותנטי ואירוח מסורתי", "מסלולי סקי", "תצפית על נמל תעופה"], 1, "בשני הכפרים: שוק צבעוני, מטבח דרוזי מסורתי, וביקור בבתי מורשת עם אירוח ביתי — למשל מרכז \"אל-בדיה\" בעוספיה."],
  ["באיזו שנה אוחדו תל אביב ויפו לעיר אחת?", ["1909", "1933", "1948", "1950"], 3, "תל אביב נוסדה רשמית ב-1909 לצד יפו העתיקה. ב-1950 אוחדו השתיים לעיר אחת — תל אביב-יפו."],
  ["מה מוצג במוזיאון אנו (בית התפוצות לשעבר)?", ["תולדות מדינת ישראל מאז הקמתה", "סיפורו של העם היהודי בתפוצות לאורך הדורות", "תולדות העיר תל אביב", "אמנות מודרנית ועכשווית בלבד"], 1, "מוזיאון אנו מספר את סיפור העם היהודי — זהותו, תרבותו ומורשתו — דרך תערוכות קבע ומתחלפות ומאגרי מידע עשירים."],
  ["מדוע יש בגן לאומי בית גוברין כל כך הרבה מערות פעמון?", ["נוצרו מפעולת גלי הים", "הקרקע הגירית הרכה אפשרה חציבה בידי אדם", "נוצרו מרעידות אדמה חוזרות", "הן שרידי מכרות ברזל"], 1, "הקרקע הגירית הרכה של שפלת יהודה אפשרה לחצוב בקלות יחסית מערות ענק בצורת פעמון — ולכן מכונה האזור \"ארץ אלף המערות\"."],
  ["מה שימש הקולומבריום בבית גוברין?", ["מקדש לפולחן דתי", "מקום קינון ליונים, לצורכי מאכל ודישון", "בריכת טבילה", "אולם קבלת אורחים"], 1, "הקולומבריום הוא מערה עם מאות נישות קטנות ששימשו לגידול יונים — למאכל, לדישון השדות ואף למסחר."],
