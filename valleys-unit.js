(() => {
  ...
  function save() {
    localStorage.setItem('valleys-progress', JSON.stringify(state));
    renderProgress();
  }
  function renderProgress() {
    const value = Math.round((state.done.length / pages.length) * 100);
    $('unitPercent').textContent = value + '%';
    $('unitMeter').style.width = value + '%';
    document
  ...
  function show(index) {
    ...
    try {
      ...
      tuMap['haamakim'] = { pageIndex: current, pageTotal: pages.length, ts: Date.now() };
  ...
  document.querySelectorAll('[data-recognition]').forEach((card) => {
    ...
    card.onclick = () => {
      ...
      recognized.add(card.dataset.recognition);
      localStorage.setItem('valleys-images', JSON.stringify([...recognized]));
    };
  });
  ...
  function renderSlide() {
    const [h, p, img] = slides[slide];
    $('valleySlideStage').innerHTML =
      `${img ? `<img src="https://nisan1234-afk.github.io/jerusalem-tour/images/${img}" alt="">` : ''}<div><small>העמקים</small><h3>${h}</h3><p>${p}</p></div>`;
    $('valleySlideCount').textContent = `${slide + 1} / ${slides.length}`;
  }
  $('valleyPrevSlide').onclick = () => {
    slide = (slide - 1 + slides.length) % slides.length;
    renderSlide();
  };
  $('valleyNextSlide').onclick = () => {
    slide = (slide + 1) % slides.length;
    renderSlide();
  };
  $('valleyFullscreen').onclick = () => document.querySelector('.slide-deck').requestFullscreen?.();
  renderSlide();

  const exams = [
    {
      title: 'שאלת בגרות — היכרות עם החבל',
      parts: [
  ...
  $('valleyExamBank').innerHTML = exams
    .map(
      (q, qi) =>
        `<article class="exam-question"><h3>${q.title}</h3>${q.parts.map((p, pi) => `<div class="exam-part"><label><b>${String.fromCharCode(1488 + pi)}.</b> ${p}</label><textarea data-exam="${qi}-${pi}" data-question="${p}"></textarea><button class="check-exam">בדיקת הסעיף</button><div class="answer-feedback"></div></div>`).join('')}</article>`
    )
    .join('');
